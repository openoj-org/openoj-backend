...
const { select_user_by_id, insert_user, select_users_by_param_order } = require("./CURDs/userCURD");
...
app.listen(8088, async () => {
	await (async () => {
		...
		// 初始化全局设置
		try {
			let settings = await querySql('INSERT INTO GLOBAL_SETTINGS(allow_register, have_list) VALUES(1, 1)');
	...
	select_users_by_param_order('user_id', true)
	.then(async users => {
		if (!users.success) {
			console.log('查找用户失败');
			return;
