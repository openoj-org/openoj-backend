...
function get_allow_register(req, res, next) {
  validateFunction(
    ...
    next,
    (req, res, next) => {
      return select_global_settings().then((normalObj) => {
        res.json({
...
function user_info(req, res, next) {
  validateFunction(
    ...
    (req, res, next) => {
      let { id } = req.query;
      return select_user_by_id(id).then((normalObj) => {
...
function user_list(req, res, next) {
  validateFunction(
    ...
    async (req, res, next) => {
      ...
      return select_users_by_param_order(
        order,
        increase,
        usernameKeyword,
        start,
...
function mail_suffux_list(req, res, next) {
  validateFunction(
    ...
    next,
    (req, res, next) => {
      return select_email_suffixes().then((normalObj) => {
        select_global_settings()
          .then((setting) => {
            res.json({
              haveList: setting.haveList,
              suffixList: setting.haveList ? normalObj.suffixList : undefined,
            });
          })
          .catch((err) => {
            res.json({
...
function login(req, res, next) {
  validateFunction(
    ...
    (req, res, next) => {
      ...
      let userObj = null;
      return select_full_user_by_email(usernameOrMail).then((usr) => {
        if (usr.success) {
          userObj = {
              ...
              .then((norObj) => {
                if (norObj.success) {
                  if (norObj.userInfo.user_role == 4) {
                    ...
                    insert_cookie(userObj.id, cookienum).then((result) => {
                      if (result.success) {
                        res.json({
                          success: true,
                          message: "登录成功",
...
function logout(req, res, next) {
  validateFunction(
    ...
    (req, res, next) => {
      let { cookie } = req.body;
      return delete_cookie(cookie).then((result) => {
        res.json(result);
      });
    },
    false
...
function allow_register(req, res, next) {
  validateFunction(
    ...
    (req, res, next) => {
      ...
      return select_user_id_by_cookie(cookie).then((usr) => {
        if (usr.success) {
          let id = usr.id;
          select_user_by_id(id).then((normalObj) => {
            if (normalObj.success) {
...
function register(req, res, next) {
  validateFunction(
    ...
    next,
    (req, res, next) => {
      return select_global_settings().then((allowregister) => {
...
function prepare_mailcode(req, res, next) {
  validateFunction(
    ...
    (req, res, next) => {
      ...
      return select_user_by_email(address).then((result) => {
        if (!result.success) {
...
function verify_mailcode(req, res, next) {
  validateFunction(
    ...
    (req, res, next) => {
      ...
      return select_mail_code_by_session_id(sessionId).then((result) => {
        if (!result.success) {
          res.status(CODE_ERROR).json({
            success: false,
            message: "验证码不存在",
...
function reset_password(req, res, next) {
  validateFunction(
    ...
    (req, res, next) => {
      ...
      return select_mail_code_by_success_session_id(sessionId).then(
        (normalObj) => {
          if (normalObj.success) {
            // 验证码类型错误, 或超过 10 分钟
