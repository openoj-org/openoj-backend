...
function select_mail_code_by_session_id(id) {
  return querySql(
    `SELECT * FROM mail_sessions WHERE mail_session_id = '${id}';`
  )
    .then((mailCodes) => {
      if (!mailCodes || mailCodes.length == 0) {
        return {
...
function select_mail_code_by_success_session_id(id) {
  return querySql(
    `SELECT * FROM mail_sessions WHERE mail_success_session_id = '${id}';`
  )
    .then((mailCodes) => {
      if (!mailCodes || mailCodes.length == 0) {
        return {
...
function insert_mail_code(session_id, mail, code_number, scene) {
  let sql = `INSERT INTO mail_sessions(mail_session_id, mail, \
		       mail_code_number, mail_session_scene, mail_code_generate_time) \
