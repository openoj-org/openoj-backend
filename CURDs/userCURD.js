...
function select_users_by_param_order(
  order,
  increase,
  usernameKeyword,
  start,
  ...
  sql +=
    usernameKeyword == null
      ? ""
      : `WHERE user_name LIKE '${usernameKeyword}%' `;
...
function update_user(id, param, value) {
  ...
  let sql = "UPDATE users SET " + param + " = " + padding + value + padding;
  if (param == "user_name") {
...
function insert_cookie(id, cookie) {
  let sql =
    "INSERT INTO cookies(user_id, cookie) " + `VALUES('${id}', '${cookie}');`;
  return querySql(sql)
    .then((result) => {
      return {
        success: result.affectedRows != 0,
        message:
          result.affectedRows != 0 ? "cookie 插入成功" : "cookie 插入失败",
      };
    })
    .catch((err) => {
      return {
